...
public abstract class AbstractTaskActionHandler<T extends DefualtTaskActionCmd> extends AbsActionHandler<T> {
	...
	protected void b(T data) {
		...
		BpmTask task = (BpmTask) this.ay.get((Serializable) ((Object) data.getTaskId()));
		if (task == null) {
		...
		data.setBpmInstance((IBpmInstance) this.f.get((Serializable) ((Object) task.getInstId())));
		this.l(data);
	...
	protected void c(DefualtTaskActionCmd actionModel) {
		this.az.execute(EventType.TASK_PRE_COMPLETE_EVENT, (TaskActionCmd) actionModel);
	}
	...
	protected void c(BaseActionCmd baseActionCmd) {
		this.b((T) ((DefualtTaskActionCmd) baseActionCmd));
	}

	public void b(BaseActionCmd baseActionCmd) {
		this.a((T) ((DefualtTaskActionCmd) baseActionCmd));
	}
}